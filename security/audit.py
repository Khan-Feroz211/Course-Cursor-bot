...
class FileUploadValidator:
    ...
    DANGEROUS_SIGNATURES = {
        b"PK\x03\x04": "zip/docx",  # ZIP/DOCX header
        b"%PDF": "pdf",  # PDF
    ...
    def validate_upload(filename: str, content: bytes) -> tuple[bool, str]:
        ...
        if not any(file_lower.endswith(ext) for ext in FileUploadValidator.ALLOWED_EXTENSIONS):
            return False, f"Only PDF, DOC, DOCX, XLSX, XLS files allowed. Got: {filename}"
        ...
        if file_lower.endswith(".pdf"):
            ...
            if not content.startswith(b"PK\x03\x04"):
                return False, "Invalid DOCX file. File signature mismatch."
        elif file_lower.endswith(".doc"):
            # DOC is OLE2 format
            if not content.startswith(b"\xd0\xcf\x11\xe0") and not content.startswith(b"PK\x03\x04"):
                return False, "Invalid DOC file. File signature mismatch."
        elif file_lower.endswith((".xlsx", ".xls")):
            # Excel files are ZIP format (XLSX) or OLE2 (XLS)
            if not content.startswith(b"PK\x03\x04") and not content.startswith(b"\xd0\xcf\x11\xe0"):
                return False, "Invalid Excel file. File signature mismatch."
        
        # Check for embedded executables in PDFs (advanced)
        if file_lower.endswith(".pdf") and b"EmbeddedFile" in content:
